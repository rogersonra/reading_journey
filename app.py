...
def edit_book():
    ...
    try:
        ...
        all_values = ws.get_all_values()
        if not all_values:
            return {"ok": False, "error": "sheet empty"}, 500
        ...
        if title_col < 0 or author_col < 0:
            return {"ok": False, "error": "columns not found"}, 500

        for row_idx, row in enumerate(all_values[1:], start=2):
            rt = row[title_col].strip()  if len(row) > title_col  else ""
            ra = row[author_col].strip() if len(row) > author_col else ""
            if rt == orig_title and ra == orig_author:
                cells = []
